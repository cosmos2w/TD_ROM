...
def load_yaml_config(base_dir, net_index, stage_index, Repeat_id):
    ...
    The file should have a name like "config_Senseiver_idx_{net_index}_<date>.yaml" 
    (ignoring the date part).
    """
    # Construct pattern using the base directory
    pattern = os.path.join(str(base_dir), f"config_Senseiver_idx_{net_index}_st_{stage_index}_num_{Repeat_id}*.yaml")
...
def evaluate(u_true_phys, u_pred_phys, recon_idx):
    ...
    print(f'Before evaluation, u_true_recon.shape is {u_true_recon.shape}, u_pred_phys.shape is {u_pred_phys.shape}')
